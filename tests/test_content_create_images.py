...
from workflow.flow.content_create.utils import build_generated_image_object_key, generate_images
...
class ContentCreateImagesTest(unittest.TestCase):
    def test_generate_images_uploads_generated_urls_to_s3(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch(
                    "workflow.flow.content_create.utils.request_image",
                    side_effect=[
                        {"data": [{"url": "https://ark.example.com/cover.png"}]},
                        {"data": [{"url": "https://ark.example.com/detail.png"}]},
                    ],
                ),
                patch("workflow.flow.content_create.utils.build_s3_uploader", return_value=uploader),
            ):
