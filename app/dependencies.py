...
from app.model import validate_tenant_api_key
...
async def require_tenant_api_key(
    ...
    if not tenant_id:
        raise HTTPException(status_code=401, detail="缺少 tenant_id，当前接口无法校验租户 API key")
    if not x_api_key or not x_api_key.strip():
        raise HTTPException(status_code=401, detail="缺少 X-API-Key")
    if not validate_tenant_api_key(database_url, tenant_id, x_api_key):
